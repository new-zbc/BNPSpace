...
p = ggplot(data = df) + 
  ...
  annotate(geom = "text", x = "BNPSpace", y = 2.4, label = expression(italic(p) < 0.0001), size = 5) + 
  annotate(geom = "text", x = "SPARK", y = 1.5, label = expression(italic(p) == 0.0912), size = 5) + 
  annotate(geom = "text", x = "SpatialDE", y = 1.4, label = expression(italic(p) == 0.0315), size = 5) + 
